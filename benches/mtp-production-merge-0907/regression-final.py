...
def parity():
    ...
    for label,binary,disabled in [('old-production','baseline-bin',False),('merged-disabled','final-bin',True),('merged-default','final-bin',False)]:
        setup(binary,disabled);p,l=start(3,'final-parity-'+label)
        try:
            restore()
            ...
            s.req('/slots/0?action=save',{'filename':'final-parity-slot.bin'})
            hashes={e:sha(R/('final-parity-slot.bin'+e)) for e in ('','.draft','.spec')}
            s.req('/slots/0?action=erase',{});s.req('/slots/0?action=restore',{'filename':'final-parity-slot.bin'})
            ...
            write('final-parity-raw.json',rows)
    ...
    write('final-parity-summary.json',checks);print('CHECKS',checks,flush=True)
...
def zero():
    ...
    s.args=args;p,l=start(3,'final-zero-budget')
    try:
        restore();out,_=parallel(f['prompts'],32)
        assert all(x['timings'].get('draft_n',0)==0 and x['timings'].get('draft_n_accepted',0)==0 for x in out),out
        write('final-zero-summary.json',{'completed_requests':len(out),'all_draft_counts_zero':True});print('ZERO PASS',flush=True)
    finally:s.stop(p,l)
...
def perf():
    ...
    for depth in (0,1,3):
        for block,arm in enumerate(('baseline-bin','final-bin','final-bin','baseline-bin')):
            setup(arm);p,l=start(depth,f'final-perf-d{depth}-{block}-{arm}')
            try:
                for rep in range(3):
                    ...
                    rows.append(row);write('final-performance-raw.json',rows)
    ...
    write('final-performance-summary.json',summary);print('PERF SUMMARY',json.dumps(summary),flush=True)
