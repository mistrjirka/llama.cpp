...
s.PORT=32537;s.OUT=R;s.BIN=str(P/'build-sm70-75/bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(P/'build-sm70-75/bin')
for k in list(s.ENV):
...
if a.mode=='capacity':
 copies=2
 p,log=s.start(3,'capacity-copies2')
 try:
  slots=s.req('/slots');assert len(slots)==4
  (R/'capacity.json').write_text(json.dumps({'slots':slots,'gpu':subprocess.check_output(['nvidia-smi','--query-gpu=uuid,name,memory.used,memory.free','--format=csv,noheader'],text=True)},indent=2))
  print('CAPACITY', (R/'capacity.json').read_text(),flush=True)
...
if a.mode=='bench':
 ...
 for block,arm in enumerate(['A','B','D','C','E','E','C','D','B','A']):
  ...
  p,log=s.start(3,f'bench-{block}-{arm}')
  try:
   assert str(P/'build-sm70-75/bin/libllama') in pathlib.Path(f'/proc/{p.pid}/maps').read_text()
   for rep in range(3):
    ...
    rows.append(row);(R/'bench-results.json').write_text(json.dumps(rows,indent=2));print(arm,rep,round(wall,4),round(row['mean_tg'],3),flush=True)
  finally:s.stop(p,log)
 summary={}
 for arm in variants:
...
if a.mode=='teacher':
 ...
 for arm,limit,copies in [('A',0,1),('C',0,2),('D',2,2),('E',1,2),('A2',0,1)]:
  env=s.ENV.copy();env.update({'GROUP_FIXTURE':str(R/'teacher-fixture.json'),'GROUP_SNAPSHOTS':str(R),'GROUP_LOGITS':str(R/f'logits-{arm}.f32')})
  ...
  with (R/f'teacher-{arm}.log').open('wb') as log:
   subprocess.run(args,env=env,stdout=log,stderr=subprocess.STDOUT,check=True,timeout=120)
  if arm!='A':
   result=json.loads(subprocess.check_output([str(R/'compare-logits'),str(R/'logits-A.f32'),str(R/f'logits-{arm}.f32')],text=True))
   results.append({'arm':arm,**result});print(arm,result,flush=True)
   (R/'teacher-results.json').write_text(json.dumps(results,indent=2))
...
if a.mode=='profile':
 ...
 measurements=[]
 for arm,limit,copies in [('A',0,1),('D',2,2),('E',1,2)]:
  ...
  try:
   ...
   (R/'profile-results.json').write_text(json.dumps(measurements,indent=2))
