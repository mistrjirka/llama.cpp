...
R=pathlib.Path('/workspace/oai-qwen38-pp-lab/results/mtp-gantt-0907');F=R.parent/'parallel-refined-0907';B=R/'candidate-bin'
...
s.PORT=32652;s.OUT=R;s.BIN=str(B/'llama-server');s.ENV['LD_LIBRARY_PATH']=str(B)
for k in list(s.ENV):
...
if a.suite=='four':order=['off','stock','copy','index','both','both','index','copy','stock','off'];concurrency=4
elif a.suite=='one':order=['stock','both','both','stock'];concurrency=1
else:order=['off','off-index','off-index','off'];concurrency=4
for block,arm in enumerate(order):
 s.ENV.pop('LLAMA_EXPERIMENT_KV_INDEXED_RM',None);s.ENV.pop('LLAMA_EXPERIMENT_MTP_SKIP_PROMPT_COPY',None)
 if arm in ['index','both','off-index']:s.ENV['LLAMA_EXPERIMENT_KV_INDEXED_RM']='1'
 if arm in ['copy','both']:s.ENV['LLAMA_EXPERIMENT_MTP_SKIP_PROMPT_COPY']='1'
 depth=0 if arm.startswith('off') else 3
