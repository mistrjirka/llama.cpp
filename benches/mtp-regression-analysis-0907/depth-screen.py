...
order=['off','warm0','mtp1','mtp2','mtp3','mtp3','mtp2','mtp1','warm0','off']
for block,arm in enumerate(order):
 def args(_):
  n=0 if arm=='off' else (3 if arm=='warm0' else int(arm[-1]))
  a=base_args(n)
  if arm=='warm0':a[a.index('--spec-draft-n-max')+1]='0'
  return a
 s.args=args;p=log=None
 try:
  p,log=s.start(0,f'../mtp-regression-analysis-0907/{block}-{arm}')
  ...
  for rep in range(2):
   ...
   rows.append(row);(R/'depth-raw.json').write_text(json.dumps(rows,indent=2))
  ...
  (R/'depth-errors.json').write_text(json.dumps(errors,indent=2));print('ERROR',arm,repr(e),flush=True)
 finally:
  if p is not None:s.stop(p,log)
  s.args=base_args
...
(R/'depth-summary.json').write_text(json.dumps(summary,indent=2));print('SUMMARY',json.dumps(summary),flush=True)
