...
s.PORT=32488;s.BIN=str(R/'packed-bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(R/'packed-bin')
rows=[];expected=None
for run,arm in enumerate(['off','p16s40','p16s80','p16s160','p8s80','off']):
 s.ENV.pop('GGML_CUDA_VOLTA_Q8_MULTI',None)
 if arm!='off':
  s.ENV['GGML_CUDA_VOLTA_Q8_MULTI']='1'
  pack,splits=arm[1:].split('s')
  s.ENV['GGML_CUDA_VOLTA_Q8_MULTI_PACK']=pack
  s.ENV['GGML_CUDA_VOLTA_Q8_MULTI_SPLITS']=splits
 p,l=s.start(3,f'../parallel-experiments-0907/packed-{run}-{arm}')
 try:
  for rep in range(2):
   ...
   rows.append(row);(R/'packed-results.json').write_text(json.dumps(rows,indent=2))
