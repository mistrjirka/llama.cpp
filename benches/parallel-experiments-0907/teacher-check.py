...
z=importlib.util.spec_from_file_location('s',O/'sweep.py');s=importlib.util.module_from_spec(z);z.loader.exec_module(s);s.PORT=32485
pr=json.loads((R/'code-prompts.json').read_text());hs=pr['histories'];base=pr['prompts'];out=[];forced=None
for label in ['A','B','A']:
 d=R/('baseline-bin' if label=='A' else 'v2-bin');s.BIN=str(d/'llama-server');s.ENV['LD_LIBRARY_PATH']=str(d)
 p,l=s.start(0,f'../prefix-first-0907/teacher-{label}-{len(out)}')
 try:
  ...
  for step in range(24):
   ...
   for i in range(4):
    ...
    if step==0 and i==0:(R/f'teacher-example-{label}.json').write_text(json.dumps(r,indent=2))
  ...
  out.append({'label':label,'values':values});(R/'teacher-raw.json').write_text(json.dumps(out,indent=2))
...
(R/'teacher-summary.json').write_text(json.dumps(summary,indent=2));print(json.dumps(summary),flush=True)
