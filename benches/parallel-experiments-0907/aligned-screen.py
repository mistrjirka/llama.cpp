...
s.PORT=32487;s.BIN=str(R/'final-bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(R/'final-bin')
all_rows=[]
...
s.ENV['LLAMA_EXPERIMENT_PREFIX_FIRST']='1'
for index,(label,flags) in enumerate([
 ('aligned-base-a',{}),
 ('aligned256',{'LLAMA_EXPERIMENT_PREFILL_MS':'1000','LLAMA_EXPERIMENT_PREFILL_QUANTUM':'256'}),
 ('aligned128',{'LLAMA_EXPERIMENT_PREFILL_MS':'1000','LLAMA_EXPERIMENT_PREFILL_QUANTUM':'128'}),
 ('aligned-base-b',{})]):
 for key in ['LLAMA_EXPERIMENT_ADAPTIVE_MTP','LLAMA_EXPERIMENT_PREFILL_MS','LLAMA_EXPERIMENT_PREFILL_QUANTUM']:s.ENV.pop(key,None)
 s.ENV.update(flags);p,l=s.start(3,f'../parallel-experiments-0907/{label}')
 try:
  if label in ['baseline','adaptive','baseline-repeat']:
   ...
   (R/'aligned-results.json').write_text(json.dumps(all_rows,indent=2));print('UNIFORM',label,wall,[round(r['timings']['predicted_per_second'],2) for r in rr],flush=True)
  ...
  (R/'aligned-results.json').write_text(json.dumps(all_rows,indent=2));print('MIXED',label,wall,'maxgap',[round(r['max_visible_gap_s'],3) for r in rr],'p95',[round(r['p95_visible_gap_s'],3) for r in rr],flush=True)
 finally:s.stop(p,l)
