...
static inline void launch(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    for (int first=0; first<Q->ne[1]; first+=T) {
        const int count=std::min(T, (int)Q->ne[1]-first);
        partial_kernel<<<dim3(KVH,e.splits),128,0,stream>>>(
            (const float*)Q->data+first*q_s1,(const block_q8_0*)K->data,(const block_q8_0*)V->data,
            (const half*)M->data+first*m_s1,
            (int)K->ne[1],e.splits,count,scale,q_s1,q_s2,k_s1,k_s2,v_s1,v_s2,m_s1,
