...
for index,(label,flags) in enumerate([
 ('loose-base-a',{}),
 ('loose250',{'LLAMA_EXPERIMENT_PREFILL_MS':'250'}),
 ('loose500',{'LLAMA_EXPERIMENT_PREFILL_MS':'500'}),
 ('loose1000',{'LLAMA_EXPERIMENT_PREFILL_MS':'1000'}),
 ('loose-base-b',{})]):
 for key in ['LLAMA_EXPERIMENT_ADAPTIVE_MTP','LLAMA_EXPERIMENT_PREFILL_MS']:s.ENV.pop(key,None)
 s.ENV.update(flags);p,l=s.start(3,f'../parallel-experiments-0907/budget-{label}')
 try:
  if label in ['baseline','adaptive','baseline-repeat']:
   ...
   (R/'budget-results.json').write_text(json.dumps(all_rows,indent=2));print('UNIFORM',label,wall,[round(r['timings']['predicted_per_second'],2) for r in rr],flush=True)
  ...
  (R/'budget-results.json').write_text(json.dumps(all_rows,indent=2));print('MIXED',label,wall,'maxgap',[round(r['max_visible_gap_s'],3) for r in rr],'p95',[round(r['p95_visible_gap_s'],3) for r in rr],flush=True)
 finally:s.stop(p,l)
