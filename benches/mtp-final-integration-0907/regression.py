...
s.PORT=32706;s.OUT=R
BASE_ENV=s.ENV.copy()
for key in list(BASE_ENV):
    if key.startswith(('LLAMA_EXPERIMENT_','GGML_CUDA_VOLTA_Q8_MULTI','GGML_CUDA_VOLTA_Q8_REF')) or key in ('GGML_CUDA_VOLTA_Q8_COOP','GGML_CUDA_VOLTA_Q8_PV_F32','LD_PRELOAD','LLAMA_ARG_BACKEND_SAMPLING','LLAMA_MTP_KV_ONLY','LLAMA_MTP_BULK_HIDDEN','LLAMA_SAMPLING_VIEW','LLAMA_KV_INDEXED_RM','LLAMA_MTP_SKIP_PROMPT_COPY','LLAMA_MTP_REQUEST_BUDGET'):
        BASE_ENV.pop(key)
...
def setup(binary='candidate-bin',disable=False,interpose=False):
...
def start(depth,label):
    ...
    assert str(pathlib.Path(s.BIN).parent.resolve()/'libllama') in pathlib.Path(f'/proc/{p.pid}/maps').read_text()
    return p,l
...
def parity():
    rows=[]
    for label,binary,disabled in [('old-production','baseline-bin',False),('merged-disabled','candidate-bin',True),('merged-default','candidate-bin',False)]:
...
def bank():
    ...
    def args(depth):
        a=original_args(3)
        for k,v in [('--ctx-size','16000'),('--kv-unified-per-slot','8000'),('--parallel','1'),('--cache-ram','4096')]:a[a.index(k)+1]=v
        return a
    setup();s.args=args;p,l=start(3,'bank-control')
    try:
        s.comp(0,prompts[0],0);rows['reference']=s.comp(0,prompts[0]+tail,64)
    finally:s.stop(p,l)
    setup();s.args=args;p,l=start(3,'bank-park')
    try:
        for prompt in prompts:s.comp(0,prompt,0)
        rows['saved']=s.req('/prompt-cache?action=save',{'filename':'parked-bank.bin'})
        assert (R/'parked-bank.bin').stat().st_size>10000,rows['saved']
    finally:s.stop(p,l)
    setup();s.args=args;p,l=start(3,'bank-restart')
    try:
        rows['loaded']=s.req('/prompt-cache?action=restore',{'filename':'parked-bank.bin'})
        rows['resumed']=s.comp(0,prompts[0]+tail,64)
        assert rows['resumed']['timings']['cache_n']==len(prompts[0]),rows['resumed']
        assert rows['reference']['tokens']==rows['resumed']['tokens'],rows
    finally:s.stop(p,l)
    write('bank-result.json',rows);print('PARKED BANK PASS',rows['saved'],rows['loaded'],flush=True)

def perf():
    rows=[]
    for depth in (0,1,3):
        for block,arm in enumerate(('baseline-bin','candidate-bin','candidate-bin','baseline-bin')):
    ...
    for depth in (0,1,3):
        d={}
        for arm in ('baseline-bin','candidate-bin'):
        ...
        d['wall_reduction_percent']=100*(1-d['candidate-bin']['wall_s']['mean']/d['baseline-bin']['wall_s']['mean']);summary[str(depth)]=d
