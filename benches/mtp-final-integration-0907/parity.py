...
R=pathlib.Path('/workspace/oai-qwen38-pp-lab/results/mtp-final-integration-0907');F=R.parent/'parallel-refined-0907'
...
for label,bin,enabled in [('baseline','baseline-bin',False),('patched','candidate-bin',True),('disabled','candidate-bin',False)]:
 ...
 for k in ['LLAMA_KV_INDEXED_RM','LLAMA_MTP_SKIP_PROMPT_COPY','LLAMA_MTP_REQUEST_BUDGET']:s.ENV.pop(k,None)
 if label=='disabled':s.ENV.update({'LLAMA_KV_INDEXED_RM':'0','LLAMA_MTP_SKIP_PROMPT_COPY':'0','LLAMA_MTP_REQUEST_BUDGET':'0'})
