...
**Branch: `perf/refined-serving-0907`. Production defaults are unchanged.**
...
`llama_get_sampling_output_ith()` returns a borrowed view of the sampled token, probabilities, logits, candidate IDs and counts after one context synchronization. Existing row accessors and fallback logic remain in use. The view is invalidated by the next decode/encode, output reallocation or context destruction.
...
The publishing worktree preserves the measured sampling-view implementation, not subsequent concurrent API-hardening edits in the development worktree. The additional FP32-PV variant is kept separately opt-in; the main serving table is explicitly tied to `gated-bin`, not to a relabeled later binary.
