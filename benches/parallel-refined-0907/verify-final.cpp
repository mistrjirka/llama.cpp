...
extern "C" bool llama_get_sampling_output_ith(llama_context *c,int32_t i,llama_sampling_output *o) {
    ...
    static std::unordered_set<llama_context *> checked;
    if (checked.insert(c).second) {
        llama_sampling_output invalid{};
        auto clear = [&]() {
            require(invalid.token==LLAMA_TOKEN_NULL && !invalid.probs && !invalid.logits &&
                    !invalid.candidates && !invalid.raw_logits && !invalid.n_probs && !invalid.n_logits,
                    "invalid view must be cleared");
        };
        require(!f(nullptr,0,&invalid),"null context"); clear();
        require(!f(c,0,nullptr),"null output");
        require(!f(c,INT_MAX,&invalid),"out of range positive row"); clear();
        require(!f(c,INT_MIN,&invalid),"out of range negative row"); clear();
    }
