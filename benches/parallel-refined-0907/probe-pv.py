...
s.PORT=32527;s.OUT=R;s.BIN=str(R/'pv-bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(R/'pv-bin')
f=json.loads((R/'real-fixture.json').read_text());hs=f['histories'];heldout=f['parent'][95000:95128];rows=[]
for arm in ['off','on','off']:
 for k in list(s.ENV):
  if k.startswith('LLAMA_EXPERIMENT_') or k.startswith('GGML_CUDA_VOLTA_Q8_') and any(x in k for x in ['MULTI','REFINED','COOP','PV_F32']):s.ENV.pop(k)
 if arm=='on':s.ENV.update({'GGML_CUDA_VOLTA_Q8_MULTI':'1','GGML_CUDA_VOLTA_Q8_MULTI_PACK':'4','GGML_CUDA_VOLTA_Q8_REFINED':'1','GGML_CUDA_VOLTA_Q8_REFINED_AUTO':'1','GGML_CUDA_VOLTA_Q8_COOP':'1','GGML_CUDA_VOLTA_Q8_PV_F32':'1'})
 p,l=s.start(0,f'pv-probability-{len(rows)}-{arm}')
 try:
  ...
  rows.append({'arm':arm,'values':vals});(R/'pv-probabilities.json').write_text(json.dumps(rows,indent=2));print('probe',arm,'done',flush=True)
 finally:s.stop(p,l)
report={}
...
(R/'pv-probability-summary.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
