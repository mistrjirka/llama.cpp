...
s.PORT=32522;s.OUT=R;s.BIN=str(R/'final-bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(R/'final-bin')
...
for arm in ['off','both','off']:
 ...
 if arm=='both':s.ENV.update({'LLAMA_EXPERIMENT_SAMPLING_VIEW':'1','LLAMA_EXPERIMENT_MTP_BULK_HIDDEN':'1','LD_PRELOAD':str(R/'verify-final.so')})
 p,log=s.start(3,f'hardened-verify-{len(rows)}-{arm}')
 try:
...
(R/'hardened-validation.json').write_text(json.dumps({'summary':report,'raw':rows},indent=2));print(json.dumps(report),flush=True)
