...
s.PORT=32524;s.OUT=R;s.BIN=str(R/'final-bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(R/'final-bin')
s.ENV.update({'LLAMA_EXPERIMENT_SAMPLING_VIEW':'1','LLAMA_EXPERIMENT_MTP_BULK_HIDDEN':'1','LD_PRELOAD':str(R/'verify-final.so')})
def args(n):
...
s.args=args;p,l=s.start(3,'hardened-qwen-check')
try:
 ...
 (R/'hardened-qwen-check.json').write_text(json.dumps(rs,indent=2));print('Qwen four-slot MTP completed with view validation',flush=True)
finally:s.stop(p,l)
