...
R=pathlib.Path('/workspace/oai-qwen38-pp-lab/results/parallel-refined-0907');O=R.parent/'parallel-research-0907'
z=importlib.util.spec_from_file_location('s',O/'sweep.py');s=importlib.util.module_from_spec(z);z.loader.exec_module(s)
s.PORT=32542;s.OUT=R
...
flags={
 'off':{},
 'gpu':{'LLAMA_ARG_BACKEND_SAMPLING':'1'},
 'gpu_both':{'LLAMA_ARG_BACKEND_SAMPLING':'1','LLAMA_EXPERIMENT_SAMPLING_VIEW':'1','LLAMA_EXPERIMENT_MTP_BULK_HIDDEN':'1'},
...
if a.suite=='sampling':arms=['off','gpu','gpu_both','gpu_both','gpu','off'];binary='host-bin'
elif a.suite=='host':arms=['off','both','both','off'];binary='final-bin'
...
for block,arm in enumerate(arms):
 ...
 try:
  ...
  for rep in range(a.repeats+1):
   ...
   rows.append(row);(R/f'final-{a.suite}-results.json').write_text(json.dumps(rows,indent=2))
...
(R/f'final-{a.suite}-summary.json').write_text(json.dumps(summary,indent=2));print('SUMMARY',json.dumps(summary),flush=True)
