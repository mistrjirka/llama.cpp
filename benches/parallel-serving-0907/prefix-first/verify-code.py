...
spec=importlib.util.spec_from_file_location('screen',ROOT/'fixture.py');s=importlib.util.module_from_spec(spec);spec.loader.exec_module(s)
s.PORT=32484
bins={'A':ROOT/'baseline-bin','B':pathlib.Path('/workspace/llama-multiagent-cache/build-sm70-75/bin')}
pr=json.loads((pathlib.Path('/workspace/oai-qwen38-pp-lab/results/prefix-first-0907/code-prompts.json')).read_text());hs=pr['histories'];prompts=pr['prompts'];results=[]
def digest(p):
 h=hashlib.sha256()
 with p.open('rb') as f:
