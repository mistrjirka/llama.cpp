...
int main() {
    ...
    for(int K:{64,512,2048})for(int T:{9,17,65,259})for(int AC:{1,2}) {
        const int M=128,E=4,U=2;size_t stride=(2+17*K/32)*M,wb=stride*E;
        ...
        std::vector<int>ids(T*U);for(int t=0;t<T;++t)for(int u=0;u<U;++u)ids[t*U+u]=(t%13<9?u:(t+u)%E);
        ...
        CUDA_CHECK(cudaMalloc(&map,E*T*4));CUDA_CHECK(cudaMalloc(&counts,E*4));CUDA_CHECK(cudaMalloc(&nt,4));CUDA_CHECK(cudaMalloc(&tiles,maxt*sizeof(pxq4_tile)));
        ...
        pxq4_build_map<<<E,256>>>(di,U,T,U,map,counts);pxq4_build_tiles<<<1,32>>>(counts,E,tiles,nt);
        ...
        printf("K=%d T=%d AC=%d peak_rel=%.3g rms_rel=%.3g\n",K,T,AC,err/peak,sqrt(se/ss));fflush(stdout);
        for(auto p:{(void*)dw,(void*)dx,(void*)di,(void*)d,(void*)dr,(void*)wf,(void*)map,(void*)counts,(void*)nt,(void*)tiles})CUDA_CHECK(cudaFree(p));
    }
    puts(failed?"PREFILL_PARITY_FAIL":"PREFILL_PARITY_PASS");return failed?1:0;
}
