...
int main(int argc,char**argv){
 try {
  ...
  int count=0;for(auto &sample:data){if(argc>6&&count>=std::stoi(argv[6]))break;++count;
   ...
   else{std::string text=sample.at("text");tokens.resize(text.size()+16);int n=llama_tokenize(vocab,text.data(),text.size(),tokens.data(),tokens.size(),true,true);if(n<0)throw std::runtime_error("tokenizer size");tokens.resize(n);}
