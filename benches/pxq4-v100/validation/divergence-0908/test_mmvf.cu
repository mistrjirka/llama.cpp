...
#include <stdexcept>
#include <cstdlib>
static const double book[] = {-0x1.f9cp-1,-0x1.788p-1,-0x1.1ep-1,-0x1.adcp-2,-0x1.344p-2,-0x1.8e4p-3,-0x1.874p-4,0,0x1.5bp-4,0x1.5ecp-3,0x1.0c4p-2,0x1.714p-2,0x1.e28p-2,0x1.338p-1,0x1.88p-1,1};
...
static double dot(const unsigned char *w,int r,int K,const float *x) {
    auto p=w+(r/64)*(128+1088*(K/32)); int rr=r%64;
    half ah;memcpy(&ah,p+rr*2,2);double anchor=__half2float(ah), s=0;
    for(int k=0;k<K;++k) {
        auto b=p+128+(k/32)*1088;int j=k%32;
        int code=(b[64+rr*16+j/2]>>(4*(j%2)))&15;
        int scale=(b[rr]>>(j<16?0:4))&15;
        s+=anchor*sub[scale]*book[code]*x[k];
    } return s;
}
...
int main(int argc,char **argv) {
    ...
    for(int T: (compact?std::vector<int>{1,8}:std::vector<int>{1,2,3,4,5,6,7,8})) for(int op:{-1,(int)GGML_GLU_OP_SWIGLU,(int)GGML_GLU_OP_GEGLU,(int)GGML_GLU_OP_SWIGLU_OAI,(int)GGML_GLU_OP_SWIGLU_CLAMP}) {
        ...
        int KP=((K+511)/512)*512,as1=KP+7,as2=(routed?AC:T)*as1+11,as3=(routed?T:AC)*as2+13;
        std::vector<float>x(as3*S);for(auto&v:x)v=(int(rng()%255)-127)*.00731f;
        ...
        xt.type=GGML_TYPE_F32;xt.ne[0]=K;xt.ne[1]=routed?AC:T;xt.ne[2]=routed?T:AC;xt.ne[3]=S;xt.nb[0]=4;xt.nb[1]=as1*4;xt.nb[2]=as2*4;xt.nb[3]=as3*4;xt.data=dx;
        ...
        ggml_cuda_pxq4_mmvf_launch(&wt,&xt,routed?&it:nullptr,&dt,op<0?nullptr:&fusion,0);
    ...
    printf("DIRECT_F32_CPU_PASS cases=%d max_peak_relative_error=%.9g\n",cases,worst);return 0;
