...
p=argparse.ArgumentParser();p.add_argument('--suite',choices=['adaptive'],default='adaptive');a=p.parse_args()
...
s.PORT=32576;s.OUT=R;s.BIN=str(B/'llama-server');s.ENV['LD_LIBRARY_PATH']=str(B)
for k in list(s.ENV):
...
order=['kv3','adaptive3','adaptive3','kv3'];concurrency=4
for block,arm in enumerate(order):
 s.ENV['LLAMA_EXPERIMENT_MTP_KV_ONLY']='1'
 s.ENV.pop('LLAMA_EXPERIMENT_ADAPTIVE_MTP',None)
 if arm=='adaptive3':s.ENV['LLAMA_EXPERIMENT_ADAPTIVE_MTP']='1'
 def arguments(_):
