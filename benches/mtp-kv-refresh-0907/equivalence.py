...
def digest(p):
 ...
 return h.hexdigest()
for label,binary,enabled in [('baseline','baseline-bin',False),('cache-only','candidate-bin',True),('disabled-control','candidate-bin',False)]:
 ...
 try:
  ...
  rows.append(row);(R/'equivalence-raw.json').write_text(json.dumps(rows,indent=2))
...
(R/'equivalence-summary.json').write_text(json.dumps(summary,indent=2));print('SUMMARY',json.dumps(summary),flush=True)
