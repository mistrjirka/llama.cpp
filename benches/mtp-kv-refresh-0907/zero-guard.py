...
p=argparse.ArgumentParser();p.add_argument('--suite',choices=['zero-guard'],default='zero-guard');a=p.parse_args()
...
s.PORT=32578;s.OUT=R;s.BIN=str(B/'llama-server');s.ENV['LD_LIBRARY_PATH']=str(B)
for k in list(s.ENV):
...
order=['off','old0','new0','new0','old0','off'];concurrency=4
for block,arm in enumerate(order):
 s.ENV['LLAMA_EXPERIMENT_MTP_KV_ONLY']='1'
 B=R/('candidate-bin' if arm=='old0' else 'final-bin')
