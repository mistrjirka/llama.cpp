...
for label,binary,enabled in [('baseline','candidate-bin',True),('final','final-bin',True)]:
 ...
 p,log=s.start(3,'final-equiv-'+label)
 try:
  ...
  rows.append(row);(R/'final-equivalence-raw.json').write_text(json.dumps(rows,indent=2))
...
(R/'final-equivalence-summary.json').write_text(json.dumps(summary,indent=2));print('SUMMARY',json.dumps(summary),flush=True)
