...
R=pathlib.Path(os.environ.get('MTP_BUDGET_RESULTS_DIR', '/workspace/oai-qwen38-pp-lab/results/mtp-warm-budget-0907'))
...
s.PORT=32593;s.OUT=R;s.BIN=str(R/'candidate-bin/llama-server');s.ENV['LD_LIBRARY_PATH']=str(R/'candidate-bin')
for k in list(s.ENV):
 if k.startswith('LLAMA_EXPERIMENT_') or k.startswith('GGML_CUDA_VOLTA_Q8_MULTI') or k.startswith('GGML_CUDA_VOLTA_Q8_REF') or k in ['LD_PRELOAD','GGML_CUDA_VOLTA_Q8_COOP','GGML_CUDA_VOLTA_Q8_PV_F32']:s.ENV.pop(k)
