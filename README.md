# llama.cpp SM70 / SM75 optimized fork

`v100-optimized` is a [`llama.cpp`](https://github.com/mistrjirka/llama.cpp) branch tuned for long-context inference on NVIDIA Volta (SM70) and Turing (SM75), especially the Tesla V100 and RTX 2080 Ti. The main workload is Qwen3.8-27B with a large reusable coding/agent context. The branch also includes tuning for Ornith-1.5-35B-A3B, MTP batching, and long-session prompt-cache changes.
...
The branch supports both SM70/Volta and SM75/Turing. The `GGML_CUDA_VOLTA_*` controls are SM70-specific and should be left unset on SM75. Build for `70`, `75`, or `70;75` for a mixed V100 + RTX 2080 Ti system.

If you run both model classes, keep separate normal and FORCE_MMQ binaries.

## Key long-context results (single V100 / SM70)

The performance numbers below are single-GPU V100 (SM70) results and are the reproducible reference point. SM75/Turing is supported, but no unmeasured SM75 speedup is implied by these numbers.

`PP` is prompt-processing/prefill throughput. `TG` is token-generation/decode throughput. All values are tokens/s.

| workload | mode | reference | optimized | change |
|---|---|---:|---:|---:|
| **Qwen3.8-27B**, 100k + 1k prompt + 256 generated, **1x V100** | MTP `n-max=3`; generation paths off → on | 410.750 PP / 27.591 TG | **413.418 PP / 36.471 TG** | **+0.65% PP / +32.19% TG** |
| **Qwen3.8-27B**, 100k KV + 1024 PP + 512 TG, **1x V100** | MTP off; 256x256 FA | 276.063 PP / 16.269 TG | **352.926 PP / 16.366 TG** | **+27.84% PP / +0.60% TG** |
| **Ornith-1.5-35B-A3B**, 100k KV + 1024 PP + 512 TG, **1x V100** | MTP off; 256x256 FA | 485.493 PP / 59.262 TG | **580.357 PP / 59.386 TG** | **+19.54% PP / +0.21% TG** |

The first row compares this fork with its optional Qwen generation paths disabled and enabled. The other rows compare the upstream/reference build with the optimized branch. All measurements use one V100; see [Benchmarks](#benchmarks) for exact setups.
...
For a mixed V100 + RTX 2080 Ti system, compile with `-DCMAKE_CUDA_ARCHITECTURES='70;75'` and retune layer/tensor placement. The single-V100 table above is the reproducible benchmark reference.
...
These controls do not replace the SM70 Volta paths. The benchmark tables in this README intentionally contain only single-V100 measurements.
