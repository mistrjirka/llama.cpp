...
static bool test_previous_recurrent_snapshot(const common_params & params, llama_model * model, const int n_vocab, uint8_t fill) {
    auto cparams = common_context_params_to_llama(params);
    cparams.n_seq_max  = 2;
    ...
    for (int token = 0; token < n_vocab; ++token) {
        const float diff = logit_diff(logits_control[token], logits_restored[token]);
        if (diff > max_diff) {
            max_diff = diff;
...
static int test_rollback(const common_params & params, llama_model * model, uint8_t fill) {
    ...
    if (!test_previous_recurrent_snapshot(params, model, n_vocab, fill)) {
        return 1;
    }
