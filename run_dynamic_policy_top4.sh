...
layers='44,30,28,35'
outdir=dynamic-policy-top4
mkdir -p "$outdir"
printf 'name\tthreshold\tratio\texit\tprompt_tps\tdecode_tps\tcache_mib\tadmissions\thit_rate\tupload_mib\tsha256\n' > "$outdir/results.tsv"
for spec in 'disabled|999|1.10' 'default|1.5|1.10' 'threshold2|2.0|1.10' 'threshold3|3.0|1.10' 'hysteresis|1.5|1.30'; do
  IFS='|' read -r name threshold ratio <<< "$spec"
  ...
  env -u GGML_EXPERT_CACHE_VMM -u GGML_EXPERT_CACHE_MANAGED -u GGML_MOE_STATIC_SPLIT_SLOTS -u GGML_MOE_STATIC_SPLIT_MAP \
    GGML_EXPERT_CACHE_MIB=256 GGML_MOE_DYNAMIC_SPLIT_SLOTS=8 GGML_MOE_DYNAMIC_SPLIT_LAYERS="$layers" \
    GGML_MOE_DYNAMIC_THRESHOLD="$threshold" GGML_MOE_DYNAMIC_ADMISSION_RATIO="$ratio" GGML_EXPERT_CACHE_PROFILE=1 \
    ...
    > "$out" 2> "$err" || rc=$?
  python3 - "$name" "$threshold" "$ratio" "$rc" "$err" "$out" <<'PY' | tee -a "$outdir/results.tsv"
import hashlib,re,sys
name,threshold,ratio,rc,err,out=sys.argv[1:]
...
print('\t'.join([name,threshold,ratio,rc,tps('prompt eval time'),tps('eval time'),alloc,adm,hit,uploaded,hashlib.sha256(data).hexdigest()]))
PY
  echo completed "$name"
done
