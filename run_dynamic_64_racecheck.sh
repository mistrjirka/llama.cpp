...
outdir=dynamic-fixed-placement-racecheck
mkdir -p "$outdir"
printf 'rep\texit\tdecode_tps\tadmissions\thit_rate\tupload_mib\tsha256\n' > "$outdir/results.tsv"
for rep in 1 2 3; do
  err="$outdir/r${rep}.err"; out="$outdir/r${rep}.out"
  rm -f "$err" "$out"
  rc=0
  env -u GGML_EXPERT_CACHE_VMM -u GGML_EXPERT_CACHE_MANAGED -u GGML_MOE_STATIC_SPLIT_SLOTS -u GGML_MOE_STATIC_SPLIT_MAP -u GGML_MOE_DYNAMIC_SPLIT_LAYERS \
    GGML_EXPERT_CACHE_MIB=8192 GGML_MOE_DYNAMIC_SPLIT_SLOTS=64 GGML_MOE_DYNAMIC_THRESHOLD=2.0 GGML_MOE_DYNAMIC_ADMISSION_RATIO=1.10 GGML_EXPERT_CACHE_PROFILE=1 \
    ...
    > "$out" 2> "$err" || rc=$?
  python3 - "$rep" "$rc" "$err" "$out" <<'PY' | tee -a "$outdir/results.tsv"
import hashlib,re,sys
rep,rc,err,out=sys.argv[1:]
...
print('\t'.join([rep,rc,tps('eval time'),adm,hit,uploaded,hashlib.sha256(open(out,'rb').read()).hexdigest()]))
PY
done
