...
outdir=logit-sanity-manual
mkdir -p "$outdir"
common=(
  -m "$model" -p "$prompt" -n 16 -c 512 -b 2048 -ub 512 -fa on -dev CUDA0
  -t 40 -tb 48 -fit off -ngl all -ncmoe 99
...
rm -f "$outdir/baseline.tokens" "$outdir/baseline.logits"
env "${base_env[@]}" \
...
printf 'baseline sha=%s tokens=%s logits=%s\n' "$(sha256sum "$outdir/baseline.out"|awk '{print $1}')" "$(wc -l < "$outdir/baseline.tokens")" "$(wc -c < "$outdir/baseline.logits")"
run_case() {
  local name=$1; shift
  rm -f "$outdir/$name.logits" "$outdir/$name.out" "$outdir/$name.err"
  env "${base_env[@]}" \
    GGML_COMPLETION_FORCE_TOKENS="$PWD/$outdir/baseline.tokens" \
    GGML_COMPLETION_LOGITS_DUMP="$PWD/$outdir/$name.logits" \
    "$@" \
    timeout 1800s build-v100/bin/llama-completion "${common[@]}" \
    > "$outdir/$name.out" 2> "$outdir/$name.err"
  printf '%s sha=%s logits=%s\n' "$name" "$(sha256sum "$outdir/$name.out"|awk '{print $1}')" "$(wc -c < "$outdir/$name.logits")"
}
run_case noadmit-all GGML_EXPERT_CACHE_MIB=8192 GGML_MOE_DYNAMIC_SPLIT_SLOTS=8 GGML_MOE_DYNAMIC_THRESHOLD=999
run_case noadmit-layer28 GGML_EXPERT_CACHE_MIB=64 GGML_MOE_DYNAMIC_SPLIT_SLOTS=8 GGML_MOE_DYNAMIC_SPLIT_LAYERS=28 GGML_MOE_DYNAMIC_THRESHOLD=999
run_case dynamic-layer28 GGML_EXPERT_CACHE_MIB=64 GGML_MOE_DYNAMIC_SPLIT_SLOTS=8 GGML_MOE_DYNAMIC_SPLIT_LAYERS=28 GGML_MOE_DYNAMIC_THRESHOLD=2.0
