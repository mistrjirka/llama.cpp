...
struct mtp_shortlist_cache_entry {
    int device = -1;
    std::string path;
...
struct mtp_shortlist_view {
    const int32_t * device_rows = nullptr;
    int nrows = 0;
};

static mtp_shortlist_view mtp_shortlist_device_rows(int device, int full_nrows) {
    const char * env = std::getenv("GGML_CUDA_QWEN35_MTP_SHORTLIST");
    if (env == nullptr || env[0] == '\0') {
        return {};
    }

    static std::mutex mutex;
    static std::vector<mtp_shortlist_cache_entry> entries;
    std::lock_guard<std::mutex> lock(mutex);

    for (const auto & entry : entries) {
        if (entry.device == device && entry.path == env) {
            return {entry.device_rows, entry.nrows};
        }
    }

    std::ifstream input(env, std::ios::binary | std::ios::ate);
    if (!input) {
        GGML_LOG_WARN("CUDA MTP shortlist: cannot open row map '%s'; using full head\n", env);
        return {};
    }
    const std::streamsize bytes = input.tellg();
    if (bytes <= 0 || bytes % (std::streamsize) sizeof(int32_t) != 0) {
        GGML_LOG_WARN("CUDA MTP shortlist: row map '%s' has invalid byte count %lld; using full head\n",
            env, (long long) bytes);
        return {};
    }
    const int nrows = (int) (bytes / (std::streamsize) sizeof(int32_t));
    if (nrows < 16384 || nrows >= full_nrows) {
        GGML_LOG_WARN("CUDA MTP shortlist: row map '%s' has %d rows; expected [16384,%d); using full head\n",
            env, nrows, full_nrows);
        return {};
    }
    input.seekg(0, std::ios::beg);

    std::vector<int32_t> rows(nrows);
    if (!input.read(reinterpret_cast<char *>(rows.data()), bytes)) {
        GGML_LOG_WARN("CUDA MTP shortlist: failed to read row map '%s'; using full head\n", env);
        return {};
    }
    int32_t prev = -1;
    for (const int32_t row : rows) {
        if (row <= prev || row < 0 || row >= full_nrows) {
            GGML_LOG_WARN("CUDA MTP shortlist: row map must be strictly increasing in [0,%d); using full head\n", full_nrows);
            return {};
        }
        prev = row;
    }
    ...
    int32_t * device_rows = nullptr;
    CUDA_CHECK(cudaMalloc(&device_rows, rows.size() * sizeof(int32_t)));
    CUDA_CHECK(cudaMemcpy(device_rows, rows.data(), rows.size() * sizeof(int32_t), cudaMemcpyHostToDevice));
    entries.push_back({device, env, device_rows, nrows});
    GGML_LOG_INFO("CUDA MTP shortlist: loaded %d mapped output-head rows from '%s' on device %d\n",
        nrows, env, device);
    return {device_rows, nrows};
}

__global__ static void mtp_shortlist_fill_neg_inf(float * dst, size_t n) {
    const size_t i = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
    if (i < n) {
        dst[i] = __int_as_float(0xff800000);
    }
}

template <int ncols_dst>
__launch_bounds__(calc_nwarps(GGML_TYPE_Q6_K, ncols_dst, MMVQ_PARAMETERS_GENERIC)*ggml_cuda_get_physical_warp_size(), 1)
static __global__ void mul_mat_vec_q6_K_mapped_rows(
        const void * vx_ptr, const void * vy_ptr, const int32_t * row_map, float * dst,
        const uint32_t ncols_x, const uint32_t stride_row_x, const uint32_t stride_col_y,
        const uint32_t stride_col_dst, const uint32_t nrows_map) {
    ...
    const int tid = warp_size * threadIdx.y + threadIdx.x;
    const int logical_row0 = rows_per_cuda_block * blockIdx.x;
    const int blocks_per_row_x = ncols_x / qk;

    int physical_rows[rows_per_cuda_block];
#pragma unroll
    for (int i = 0; i < rows_per_cuda_block; ++i) {
        const int logical_row = logical_row0 + i;
        physical_rows[i] = logical_row < (int) nrows_map ? row_map[logical_row] : 0;
    }

    const block_q8_1 * y = static_cast<const block_q8_1 *>(vy_ptr);
    float tmp[ncols_dst][rows_per_cuda_block] = {{0.0f}};
...
#pragma unroll
            for (int i = 0; i < rows_per_cuda_block; ++i) {
                tmp[j][i] += vec_dot_q_cuda(
                    vx_ptr, &y[j * stride_col_y + kby], physical_rows[i] * stride_row_x + kbx, kqs);
            }
        }
    }
...
#pragma unroll
            ...
            tmp[j][i] = warp_reduce_sum<warp_size>(tmp[j][i]);
            if (threadIdx.x == i) {
                dst[j * stride_col_dst + physical_rows[i]] = tmp[j][i];
            }
        }
    }
}
...
template <int ncols_dst>
static void launch_mtp_shortlist_q6_K(
        const void * vx, const void * vy, const int32_t * row_map, float * dst,
        int ncols_x, int nrows_map, int stride_row_x, int stride_col_y, int stride_col_dst, cudaStream_t stream) {
    constexpr int nwarps = calc_nwarps(GGML_TYPE_Q6_K, ncols_dst, MMVQ_PARAMETERS_GENERIC);
    constexpr int rows_per_cuda_block = calc_rows_per_block(ncols_dst, MMVQ_PARAMETERS_GENERIC, false, nwarps);
    const dim3 grid((nrows_map + rows_per_cuda_block - 1) / rows_per_cuda_block, 1, 1);
    const dim3 block(WARP_SIZE, nwarps, 1);
    const ggml_cuda_kernel_launch_params params(grid, block, 0, stream);
    ggml_cuda_kernel_launch(mul_mat_vec_q6_K_mapped_rows<ncols_dst>, params,
        vx, vy, row_map, dst, ncols_x, stride_row_x, stride_col_y, stride_col_dst, nrows_map);
}

static void launch_mtp_shortlist_q6_K_switch(
        const void * vx, const void * vy, const int32_t * row_map, float * dst, int ncols_x,
        int ncols_dst, int nrows_map, int stride_row_x, int stride_col_y, int stride_col_dst, cudaStream_t stream) {
    switch (ncols_dst) {
        case 1: launch_mtp_shortlist_q6_K<1>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 2: launch_mtp_shortlist_q6_K<2>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 3: launch_mtp_shortlist_q6_K<3>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 4: launch_mtp_shortlist_q6_K<4>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 5: launch_mtp_shortlist_q6_K<5>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 6: launch_mtp_shortlist_q6_K<6>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 7: launch_mtp_shortlist_q6_K<7>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        case 8: launch_mtp_shortlist_q6_K<8>(vx, vy, row_map, dst, ncols_x, nrows_map, stride_row_x, stride_col_y, stride_col_dst, stream); break;
        default: GGML_ABORT("MTP shortlist only supports 1..8 columns");
    }
}

} // namespace

bool ggml_cuda_try_mul_mat_vec_q_mtp_shortlist(
        ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    const int device = ggml_cuda_get_device();
    const int cc = ggml_cuda_info().devices[device].cc;
    if (cc != GGML_CUDA_CC_VOLTA || src0->type != GGML_TYPE_Q6_K || src1->type != GGML_TYPE_F32 || dst->type != GGML_TYPE_F32) {
        return false;
    }
    // Exact Qwen3.5/3.8 27B output-head geometry. Keep this deliberately narrow until validated elsewhere.
    if (src0->ne[0] != 5120 || src0->ne[1] != 248320 || src0->ne[2] != 1 || src0->ne[3] != 1 ||
            src1->ne[0] != 5120 || src1->ne[1] < 1 || src1->ne[1] > MMVQ_MAX_BATCH_SIZE ||
            src1->ne[2] != 1 || src1->ne[3] != 1 || dst->ne[0] != 248320 || dst->ne[1] != src1->ne[1] ||
            dst->ne[2] != 1 || dst->ne[3] != 1) {
        return false;
    }

    const mtp_shortlist_view shortlist = mtp_shortlist_device_rows(device, (int) src0->ne[1]);
    if (shortlist.device_rows == nullptr) {
        return false;
    }

    cudaStream_t stream = ctx.stream();
    const int64_t ne10_padded = GGML_PAD(src1->ne[0], MATRIX_ROW_PADDING);
    ggml_cuda_pool_alloc<char> src1_q8_1(ctx.pool(), src1->ne[1] * ne10_padded * sizeof(block_q8_1) / QK8_1);
    quantize_row_q8_1_cuda(
        static_cast<const float *>(src1->data), nullptr, src1_q8_1.get(), src0->type,
    ...
    const size_t nout = (size_t) dst->ne[0] * (size_t) dst->ne[1];
    constexpr int fill_threads = 256;
    mtp_shortlist_fill_neg_inf<<<(nout + fill_threads - 1) / fill_threads, fill_threads, 0, stream>>>(
        static_cast<float *>(dst->data), nout);
    CUDA_CHECK(cudaGetLastError());

    const int stride_row_x = src0->nb[1] / ggml_type_size(src0->type);
    const int stride_col_y = ne10_padded / QK8_1;
    const int stride_col_dst = dst->nb[1] / sizeof(float);
    launch_mtp_shortlist_q6_K_switch(
        src0->data, src1_q8_1.get(), shortlist.device_rows, static_cast<float *>(dst->data), (int) src0->ne[0],
        (int) src1->ne[1], shortlist.nrows, stride_row_x, stride_col_y, stride_col_dst, stream);
    CUDA_CHECK(cudaGetLastError());
    return true;
}
...
void ggml_cuda_mul_mat_vec_q(
    ...
    if (ids == nullptr && fusion == nullptr && ggml_get_op_params_i32(dst, 1) == GGML_HINT_MTP_SHORTLIST &&
            ggml_cuda_try_mul_mat_vec_q_mtp_shortlist(ctx, src0, src1, dst)) {
        return;
    }
