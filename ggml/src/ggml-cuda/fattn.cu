...
static best_fattn_kernel ggml_cuda_get_best_fattn_kernel(const int device, const ggml_tensor * dst) {
    ...
    // Experimental direct-Q8 route for Ornith's packed multi-agent attention.
    if (cc == GGML_CUDA_CC_VOLTA && std::getenv("GGML_CUDA_VOLTA_Q8_MULTI") != nullptr &&
            Q->ne[0] == 256 && Q->ne[1] >= 1 && Q->ne[1] <= 16 && Q->ne[2] == 16 && Q->ne[3] == 1 &&
            K->type == GGML_TYPE_Q8_0 && V->type == GGML_TYPE_Q8_0 &&
