...
// Volta Q8 GQA2 decode kernel for D=256.
//
// One CTA handles two query heads which share the same K/V head. Q is quantized to q8_1
// in shared memory, each q8_0 K fragment is loaded once and dotted against both Q heads,
// and each q8_0 V element is dequantized once and accumulated into both outputs.
// This avoids the stock one-CTA-per-Q-head kernel rereading the same GQA cache twice,
// while also avoiding its 255-register/thread footprint.
...
static __global__ void flash_attn_ext_vec_q8_gqa(
    ...
    static_assert(D == 256, "Volta Q8 GQA kernel is specialized for D=256");
...
void ggml_cuda_flash_attn_ext_vec_case_impl(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if constexpr (D == 256 && cols_per_block == 1 &&
        ...
        const int gqa_ratio = K->ne[2] > 0 ? int(Q->ne[2] / K->ne[2]) : 1;
        const bool qwen38_gqa6 =
            (Q->ne[2] == 24 && K->ne[2] == 4) ||
            (Q->ne[2] == 12 && K->ne[2] == 2);

        bool gqa2_enabled = true;
        if (const char * e = getenv("GGML_CUDA_VOLTA_Q8_GQA2")) {
            gqa2_enabled = atoi(e) != 0;
        }

        if (gqa2_enabled &&
                cc == GGML_CUDA_CC_VOLTA &&
                Q->ne[1] == 1 && Q->ne[3] == 1 &&
                gqa_ratio == 6 && qwen38_gqa6 &&
                dst->src[4] == nullptr && max_bias == 0.0f) {
            constexpr int gqa_nwarps = D / WARP_SIZE;
            constexpr size_t nbytes_shared_gqa = 0;
            launch_fattn<D, 1, 2>(ctx, dst, flash_attn_ext_vec_q8_gqa<D, 2>,
                gqa_nwarps, nbytes_shared_gqa, D, false, false, false, false);
            return;
        }
    }
