...
static inline void launch_partial(
    ...
    constexpr size_t shared_bytes = ((NT*G + 2*BC)*SMEM_STRIDE + NT*BC)*sizeof(half);
    static bool configured[GGML_CUDA_MAX_DEVICES] = {};
    if (!configured[ctx.device]) {
        CUDA_CHECK(cudaFuncSetAttribute(partial_kernel<NT>, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_bytes));
        configured[ctx.device] = true;
    }
    partial_kernel<NT><<<dim3(KVH,e.splits),NT*32,shared_bytes,ctx.stream()>>>(
...
static inline void launch(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    for (int first=0; first<Q->ne[1]; first+=pack) {
        ...
        if (count <= 4) launch_partial<4>(ctx,Q,K,V,M,e,first,count,scale);
        else if (count <= 8) launch_partial<8>(ctx,Q,K,V,M,e,first,count,scale);
        else launch_partial<16>(ctx,Q,K,V,M,e,first,count,scale);
